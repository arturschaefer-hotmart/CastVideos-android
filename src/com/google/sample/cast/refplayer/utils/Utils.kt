...
object Utils {
    ...
    fun showQueuePopup(context: Context, view: View?, mediaInfo: MediaInfo?) {
        ...
        popup.menuInflater.inflate(
                if (provider.isQueueDetached || provider.count == 0) R.menu.detached_popup_add_to_queue else R.menu.popup_add_to_queue, popup.menu)
        val clickListener = PopupMenu.OnMenuItemClickListener { menuItem ->
            ...
            if (provider.isQueueDetached && provider.count > 0) {
                if (menuItem.itemId == R.id.action_play_now
                        || menuItem.itemId == R.id.action_add_to_queue) {
                    val items = rebuildQueueAndAppend(provider.items, queueItem)
                    remoteMediaClient.queueLoad(items, provider.count,
                ...
                if (provider.count == 0) {
                    ...
                    val currentId = provider.currentItemId
                    if (menuItem.itemId == R.id.action_play_now) {
                        remoteMediaClient.queueInsertAndPlayItem(queueItem, currentId, null)
                    } else if (menuItem.itemId == R.id.action_play_next) {
                        val currentPosition = provider.getPositionByItemId(currentId)
                        if (currentPosition == provider.count - 1) { //we are adding to the end of queue
                            remoteMediaClient.queueAppendItem(queueItem, null)
                        } else {
                            val nextItemId = provider.getItem(currentPosition + 1).itemId
    ...
    fun rebuildQueue(items: List<MediaQueueItem?>?): Array<MediaQueueItem?>? {
        ...
        val rebuiltQueue = arrayOfNulls<MediaQueueItem>(items.size)
        for (i in items.indices) {
            rebuiltQueue[i] = rebuildQueueItem(items[i])
        }
        return rebuiltQueue
    }
    ...
    fun rebuildQueueAndAppend(items: List<MediaQueueItem?>?,
        ...
        val rebuiltQueue = arrayOfNulls<MediaQueueItem>(items.size + 1)
        for (i in items.indices) {
            rebuiltQueue[i] = rebuildQueueItem(items[i])
        }
    ...
    fun rebuildQueueItem(item: MediaQueueItem?): MediaQueueItem {
