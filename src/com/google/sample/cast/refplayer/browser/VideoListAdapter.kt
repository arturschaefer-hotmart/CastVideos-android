...
class VideoListAdapter(private val mClickListener: ItemClickListener, context: Context) : RecyclerView.Adapter<VideoListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.mMenu.setOnClickListener { view -> mClickListener.itemClicked(view, item, position) }
        viewHolder.mImgView.setOnClickListener { view -> mClickListener.itemClicked(view, item, position) }
        viewHolder.mTextContainer.setOnClickListener { view -> mClickListener.itemClicked(view, item, position) }
        val castSession = CastContext.getSharedInstance(mAppContext).sessionManager
                .currentCastSession
        viewHolder.mMenu.visibility = if (castSession != null && castSession.isConnected) View.VISIBLE else View.GONE
    }
    ...
    class ViewHolder private constructor(private val mParent: View, val mImgView: NetworkImageView, val mTextContainer: View, private val mTitleView: TextView,
                                         private val mDescriptionView: TextView, val mMenu: View) : RecyclerView.ViewHolder(mParent) {
